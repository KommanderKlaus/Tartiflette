...
void Mesh::printstructure(const std::string& filename) {
  ...
  do {
    do {  
		...
		ofs << "| image by alpha0 : "<< following->alpha0()->getVertex().getx() << " "<< following->alpha0()->getVertex().gety()<<" " << following->alpha0()->getVertex().getz();  
		ofs << "| image by alpha1 : "<< following->alpha1()->getVertex().getx()<< " " << following->alpha1()->getVertex().gety()<< " " << following->alpha1()->getVertex().getz();
		ofs << "| image by alpha2 : "<< following->alpha2()->getVertex().getx()<< " " << following->alpha2()->getVertex().gety()<< " " << following->alpha2()->getVertex().getz()<<std::endl; 
